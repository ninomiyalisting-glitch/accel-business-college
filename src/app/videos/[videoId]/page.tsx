  ...
  return (
    ...
    >
      <div className="relative w-28 aspect-video rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
        ...
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <div className="w-7 h-7 rounded-full bg-white/90 flex items-center justify-center">
            <Play size={11} className="text-[#006899] ml-0.5" fill="currentColor" />
          </div>
        </div>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-gray-800 leading-relaxed line-clamp-2 group-hover:text-[#006899] transition-colors">
          {video.name}
        </p>
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="w-10 h-10 border-[3px] border-accel-dark border-t-[#007cb4] rounded-full animate-spin" />
      </div>
    )
  }
          ...
          <div className="mt-5 space-y-3">
            ...
            {video.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {video.tags.map((t) => (
                  <span key={t.name} className="flex items-center gap-1 text-xs px-2.5 py-1 bg-[#007cb4]/20 text-accel-light rounded-full">
                    <Tag size={10} />{t.name}
                  </span>
                ))}
              </div>

  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f3f8fc] flex items-center justify-center">
        <div className="w-8 h-8 border-3 border-[#007cb4]/30 border-t-[#007cb4] rounded-full animate-spin" />
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#f3f8fc]">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10 pt-[env(safe-area-inset-top)]">
        <div className="max-w-content mx-auto px-4 h-14 flex items-center gap-3">
          ...
          {isOwnProfile && (
            <Link
              href="/members/edit"
              className="ml-auto flex items-center gap-1.5 px-3 py-1.5 bg-[#006899] hover:bg-[#004f75] text-white rounded-lg text-xs font-medium transition-colors"
            >
              <Pencil size={12} />
              プロフィールを編集
            </Link>
      ...
      <main className="max-w-content mx-auto px-4 pt-6 pb-bottom-nav">
        ...
        {isOwnProfile && !profile && (
          <div className="bg-white rounded-2xl border border-dashed border-[#007cb4]/30 p-8 text-center">
            <p className="text-gray-500 text-sm mb-4">プロフィールを充実させてメンバーに自己紹介しましょう</p>
            <Link
              href="/members/edit"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#006899] hover:bg-[#004f75] text-white rounded-xl text-sm font-medium transition-colors"
            >
              <Pencil size={14} />
              プロフィールを入力する
            </Link>

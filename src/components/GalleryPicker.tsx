  ...
  return (
    <div className="space-y-3">
      <select
        ...
        className="w-full text-sm text-gray-800 border border-gray-200 rounded-xl px-3 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-[#007cb4]/20 focus:border-[#007cb4]"
      >
        {categories.map((c) => (
            ...
            return (
              <button
                ...
                className={`relative aspect-[4/3] rounded-lg overflow-hidden border-2 transition-all ${
                  isSelected ? 'border-[#007cb4] ring-2 ring-[#007cb4]/30' : 'border-gray-200 hover:border-gray-400'
                }`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.image_url} alt="" loading="lazy" decoding="async" className="w-full h-full object-cover" />
                {isSelected && (
                  <span className="absolute top-1 right-1 w-5 h-5 rounded-full bg-[#006899] flex items-center justify-center text-white">
                    <Check size={12} />
                  </span>
                )}
              </button>
